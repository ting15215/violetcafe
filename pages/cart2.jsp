...
<body>
		...
		<%
					while(rs.next())
					{
						...
						{	
							...
							out.print("<img src='"+rs_products.getString(4)+"' class='itemimg'>");//商品圖片
							out.print("<div class='card1'>");
							out.print("<p class='text' style='font-weight: 900;'>品名:"+rs_products.getString(2)+"<br>"+"</p>");//商品名稱
							out.print("價格:<span class='price'>NT"+rs_products.getString(6)+"</span><br>");//商品價格
							out.print("<div>");			
							out.print("<input type='hidden' name='product_id"+a+"' value='"+rs_products.getString(1)+"'>");//隱藏傳遞，跟表單一起傳
							out.print("<input type='button' value='-' class='btn1' onclick='minus("+a+");updateTotal();'>&nbsp");
							out.print("<input type='text' size='3' class='input_num' name='product_number"+a+"' value='"+rs.getString(3)+"'>&nbsp");
							out.print("<input type='button' value='+' class='btn1' onclick='add("+a+");updateTotal();'>");
							out.print("<button class='delete'> <a href='cart_delete.jsp?product_id="+rs_products.getString(1)+"' style='text-decoration:none;color:white;'>"+"刪除</a></button>");
							out.print("</div>");
							out.print("</p>");
							out.print("</div>");
							out.print("</div> ");
					...
					out.print("<div class='price' id='totalAmount'>");
					out.print("總計:NT"+sum);
					out.print("</div>");
					out.print("</div>");
    ...
    <div class="modal fade" id="loginModal">
        <div class="modal-dialog">
            ...
            <div class="modal-body">
                <!-- 個資表單 -->
                <form method="post" action="order_add.jsp">
                    <!-- 內容欄位 -->
                    <div class="form-group">
                        <input type="text" class="account form-control" name="name" placeholder="收件人姓名">
                        ...
                        <select id="pay_way" name="pay_way" class="account form-control">
    ...
    <script>
        ...
        var inputNums = document.getElementsByClassName("input_num");
        var prices = document.getElementsByClassName("price");
        var total = 0;

		for (var i = 0; i < inputNums.length; i++) {
			var num = Number(inputNums[i].value);
			var priceStr = prices[i].textContent; // 獲取商品價格的完整字符串
			var price = Number(priceStr.replace(/[^0-9]/g, '')); // 從字符串中提取數字部分作為商品價格

			total += num * price;
		}

        // 更新總額顯示
		document.getElementById("totalAmount").textContent = "總計: NT" + total;
    }
