				...
				try {	// 例外處理
					...
					if(con.isClosed()) // con.isClosed() 回傳 ture 代表連線關閉
					   out.println("警告：連線建立失敗！"); // 連線關閉代表連線沒有被建立
						...
						sql="SELECT * from `products` WHERE `id` like 'P%'"; // SQL 語法：取得 counter 資料表中的所有資料
				   ...
				   while(rs.next())
				   {
					   ...
					   out.print("<a href='products.jsp?product_id="+rs.getString(1)+"'>"+"<img src='"+rs.getString(4)+"'>"+"</a>");//商品圖片
					   out.print("<h3>"+rs.getString(2)+"</h3>"+"<br>");//商品名稱
					   out.print("<p style='font-size:15px;'>"+rs.getString(3)+"</p>"+"<br>");//商品介紹
